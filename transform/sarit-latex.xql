...
module namespace model="http://www.tei-c.org/tei-simple/models/sarit.odd/latex";
...
import module namespace latex="http://www.tei-c.org/tei-simple/xquery/functions/latex" at "xmldb:exist://embedded-eXist-server/db/apps/tei-publisher-lib/content/latex-functions.xql";
    ...
    $input !         (
            typeswitch(.)
                ...
                case element(cit) return
                    latex:cit($config, ., ("tei-cit", "citation"), .)
                            ...
                            if (parent::lg) then
                                ...
                                if (parent::list) then
                                    ...
                                    if (preceding-sibling::head) then
                                        latex:omit($config, ., ("tei-head5"), .)
                                    else
                                        if (not(preceding-sibling::head)) then
                                            latex:heading($config, ., ("tei-head6"), (node(), for $h in ./following-sibling::head return (" - ", $h/node())))
                                        else
                                            if (parent::div) then
                                                latex:heading($config, ., ("tei-head7"), .)
                                            else
                                                latex:block($config, ., ("tei-head8"), .)
                ...
                case element(TEI) return
                    ext-latex:document($config, ., ("tei-TEI"), .)
